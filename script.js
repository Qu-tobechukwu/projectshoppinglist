/* script.js - frontend for Stellies PWA + Netlify backend */
const DATA_URL = '/data/products.json';
// Netlify function endpoint (change if needed)
const BACKEND_URL = '/.netlify/functions/order';
...
function renderProducts(page=1){
  ...
  list.forEach(p=>{
    ...
    if(p.flavours && p.flavours.length){
      ...
      chk.addEventListener('change', ()=>{
        if(chk.checked){ cart = cart.filter(c=> !(c.productId===p.id && c.type==='food')); cart.push({ productId:p.id, itemName:p.name, type:'food', flavour:'', qty:Number(qty.value)||1, price:Number(p.price) }); }
        else cart = cart.filter(c=> !(c.productId===p.id && c.type==='food'));
        saveCart(); updateTotals();
      });
      qty.addEventListener('input', ()=>{ cart = cart.map(c=> (c.productId===p.id && c.type==='food' && c.flavour==='')?{...c,qty:Number(qty.value)||1}:c); saveCart(); updateTotals(); });
    }
  });

  // attach flavour handlers
  ...
  document.querySelectorAll('.flv-qty').forEach(qel=>{
    qel.addEventListener('input', ()=>{
      const id = Number(qel.dataset.id), fl = qel.dataset.fl; const qty = Math.max(0, Number(qel.value||0));
...
function renderMerch(){
  ...
  document.querySelectorAll('.add-merch').forEach(b=> b.addEventListener('click', ()=>{
    ...
    saveCart(); updateTotals(); b.textContent='Added ✓'; setTimeout(()=>b.textContent='Add to cart',700);
  }));
}
...
function checkoutInit(){
  ...
  // populate addresses
  const sel = $('delivery'); if(sel){ sel.innerHTML=''; (dataStore.addresses||[]).forEach(a=>{ const opt=document.createElement('option'); opt.value=a; opt.textContent=a; sel.appendChild(opt); }); }
  ...
  payBtn && payBtn.addEventListener('click', async ()=>{
    ...
    try{
      ...
      console.error(err);
      // fallback: save pending locally and download JSON
...
document.addEventListener('click', e=>{
  ...
  if(e.target && e.target.id === 'installBtn'){ if(deferredPrompt){ deferredPrompt.prompt(); deferredPrompt.userChoice.then(()=>{ deferredPrompt = null; $('installBtn').style.display='none'; }); } }
});
...
window.addEventListener('beforeinstallprompt', (e)=>{
  ...
  // auto-show after 6 seconds (user wanted auto prompt)
  setTimeout(()=>{ if(deferredPrompt){ deferredPrompt.prompt(); deferredPrompt.userChoice.then(()=>{ deferredPrompt = null; if($('installBtn')) $('installBtn').style.display='none'; }); } }, 6000);
});
